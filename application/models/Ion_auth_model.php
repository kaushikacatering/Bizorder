...
// 			$this->tenantDb = $this->load->database($group_name, TRUE, TRUE);
// 		}   
	...
	public function activate($id, $code = FALSE)
	{
		...
		if ($code === FALSE || ($user && $user->id === $id))
		{
			...
			if ($this->tenantDb->affected_rows() === 1)
			{
	...
	public function deactivate($id = NULL)
	{
		...
		$return = $this->tenantDb->affected_rows() == 1;
		if ($return)
		{
	...
	public function clear_forgotten_password_code($identity) {
		...
		$this->tenantDb->update($this->tables['users'], $data, [$this->identity_column => $identity]);
	...
	public function change_password($identity, $old, $new)
	{
		...
		$query = $this->tenantDb->select('id, password')
	...
	public function username_check($username = '')
	{
		...
		return $this->tenantDb->where('username', $username)
						->limit(1)
	...
	public function email_check($email = '')
	{
		...
		return $this->tenantDb->where('email', $email)
						->limit(1)
						->count_all_results($this->tables['users']) > 0;
	}
	...
	public function identity_check($identity = '')
	{
		...
		return $this->tenantDb->where($this->identity_column, $identity)
						->limit(1)
						->count_all_results($this->tables['users']) > 0;
	}
	...
	public function forgotten_password($identity)
	{
		...
		$this->trigger_events('extra_where');
		$this->tenantDb->update($this->tables['users'], $update, [$this->identity_column => $identity]);
...
// 		else
		...
		$existing_user = $this->tenantDb->get_where($this->tables['users'], ['email' => $email,'is_deleted' => 0], 1)->num_rows();
		if($existing_user !== 0)
		{
			$this->set_error('user_already_exists');
			return FALSE;
		}
		
 
		// check if the default set in config exists in database
		$query = $this->tenantDb->get_where($this->tables['groups'], ['name' => $this->config->item('default_group', 'ion_auth')], 1)->row();
		...
		// PLAIN TEXT MODE: hash_password() now just trims and returns the password
		...
		$this->tenantDb->insert($this->tables['users'], $user_data);

		$id = $this->tenantDb->insert_id($this->tables['users'] . '_id_seq');
		$locationIds = (isset($additional_data['location_ids']) ? unserialize($additional_data['location_ids']) : array());
		if(!empty($locationIds)){
		    $user_Locationdata = array();
		    foreach($locationIds as $locationIdd){
		        $user_Locationdata['user_id'] = $id;
		        $user_Locationdata['location_id'] = $locationIdd;
		        
		   	$this->tenantDb->insert('Global_users_to_location', $user_Locationdata);    
		    }
		    
		}
		// insert in locatin_table , all the location assigned to this user
	...
	public function login($identity, $password, $remember=FALSE)
	{
	   ...
	   // Ensure tenantDb is available - get from controller if not set
	   if (!isset($this->tenantDb) || !is_object($this->tenantDb)) {
			$CI = &get_instance();
			if (isset($CI->tenantDb) && is_object($CI->tenantDb)) {
				$this->tenantDb = $CI->tenantDb;
			} else {
				log_message('error', "USER LOGIN FAILED: Database connection not available. Identity={$identity}, Timestamp=" . date('Y-m-d H:i:s'));
				$this->set_error('wrong_url');
				return FALSE;
			}
		}
		...
		$query = $this->tenantDb->select($this->identity_column . ', email, id, password, active, last_login')
						  ->where($this->identity_column, $identity)
						  ->where('is_deleted', 0)
						  ->limit(1)
						  ->order_by('id', 'desc')
		...
		{
			log_message('warning', "USER LOGIN FAILED: Max login attempts exceeded. Identity={$identity}, Timestamp=" . date('Y-m-d H:i:s'));
		...
		if ($query->num_rows() === 1)
		{
			...
			{
				log_message('info', "LOGIN SUCCESS: Identity={$identity}, User ID={$user->id}, Timestamp=" . date('Y-m-d H:i:s'));
				if ($user->active == 0)
				{
					log_message('warning', "USER LOGIN FAILED: Account inactive. Identity={$identity}, User ID={$user->id}, Timestamp=" . date('Y-m-d H:i:s'));
					$this->trigger_events('post_login_unsuccessful');
			...
			// DISABLED: Rehash causes race condition when 2 users login simultaneously
			// The rehash_password_if_needed function updates passwords in DB during login
			// If 2 people login at same time, both try to update the password hash,
			// causing database corruption and subsequent login failures
			// FIX: Run PRODUCTION_LOGIN_FIX.php to convert all passwords to argon2 once,
			//      then disable auto-rehashing to prevent race conditions
			// $this->rehash_password_if_needed($user->password, $identity, $password);

			// DISABLED: Session regeneration causes conflicts with concurrent logins
			// When 2 users log in with same credentials, both try to regenerate sessions
			// This causes one user to be logged out or experience session errors
			// FIX: Rely on browser fingerprinting (added in Auth.php) and session timeout instead
			// Security note: Session fixation risk is mitigated by:
			//   1. HTTPS (cookie_secure = TRUE)
			//   2. Session expiration (7200 seconds)
			//   3. Browser fingerprinting (unique ID per browser)
		...
		// Log login failure reason
		if ($query->num_rows() === 0) {
			log_message('warning', "USER LOGIN FAILED: User not found. Identity={$identity}, Timestamp=" . date('Y-m-d H:i:s'));
		} else {
			$user = $query->row();
			log_message('warning', "USER LOGIN FAILED: Password mismatch. Identity={$identity}, User ID={$user->id}, Timestamp=" . date('Y-m-d H:i:s'));
		}
	...
	public function get_attempts_num($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			$this->tenantDb->select('1', FALSE);
			$this->tenantDb->where('login', $identity);
			if ($this->config->item('track_login_ip_address', 'ion_auth'))
			{
				...
				$this->tenantDb->where('ip_address', $ip_address);
			}
			$this->tenantDb->where('time >', time() - $this->config->item('lockout_time', 'ion_auth'), FALSE);
			$qres = $this->tenantDb->get($this->tables['login_attempts']);
	...
	public function get_last_attempt_time($identity, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$qres = $this->tenantDb->get($this->tables['login_attempts'], 1);
	...
	public function get_last_attempt_ip($identity)
	{
		...
		{
			$this->tenantDb->select('ip_address');
			$this->tenantDb->where('login', $identity);
			$this->tenantDb->order_by('id', 'desc');
			$qres = $this->tenantDb->get($this->tables['login_attempts'], 1);
	...
	public function increase_login_attempts($identity)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			return $this->tenantDb->insert($this->tables['login_attempts'], $data);
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $old_attempts_expire_period = 86400, $ip_address = NULL)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			$this->tenantDb->or_where('time <', time() - $old_attempts_expire_period, FALSE);

			return $this->tenantDb->delete($this->tables['login_attempts']);
		}
		return FALSE;
	}
	...
	public function orWhere($orwhere, $value = NULL)
	{
		$this->trigger_events('where');

		if (!is_array($orwhere))
		{
			$orwhere = [$orwhere => $value];
		}

		array_push($this->_ion_orwhere, $orwhere);
	 ...
	 */
	 ...
	 public function ConditionalUserFtehcing($isActive){ 
     $this->tenantDb->select('Global_users.*');
     $this->tenantDb->from('Global_users');
     $this->tenantDb->where('Global_users.is_deleted', 0);
     $this->tenantDb->where('Global_users.is_superadmin', 0);
     $this->tenantDb->where('Global_users.active', $isActive);
...
if ($query->num_rows() > 0) {
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			{
				$this->tenantDb->join($this->tables['groups'], $this->tables['users_groups'] . '.' . $this->join['groups'] . ' = ' . $this->tables['groups'] . '.id', 'inner');
				$this->tenantDb->where_in($this->tables['groups'] . '.name', $group_names);
			}
			if(!empty($group_ids))
			{
				$this->tenantDb->{$or_where_in}($this->tables['users_groups'].'.'.$this->join['groups'], $group_ids);
			}
		}
		...
		if (isset($this->_ion_like) && !empty($this->_ion_like))
		{
			...
			{
				$this->tenantDb->or_like($like['like'], $like['value'], $like['position']);
			}
		...
		{
			$this->tenantDb->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$this->tenantDb->limit($this->_ion_limit);
		...
		{
			$this->tenantDb->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $this->tenantDb->get($this->tables['users']);
	...
	public function get_users_groups($id = FALSE)
	{
		...
		if (!isset($this->tenantDb) || !is_object($this->tenantDb)) {
			...
			if (isset($CI->tenantDb) && is_object($CI->tenantDb)) {
				...
				// Return empty result if tenantDb not available
				$CI->load->database();
				return $CI->db->query("SELECT NULL as id, NULL as name, NULL as description WHERE 1=0");
			}
		}

		return $this->tenantDb->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE)
	{
		...
		foreach ($check_group as $key => $value)
		{
			...
			if (in_array(ucfirst($value), $groups) xor $check_all)
			{
				/**
				 * if !all (default), true
				 * if all, false
...
// echo "<pre>"; print_r($group_ids); exit;
	...
	public function remove_from_group($group_ids = FALSE, $user_id = FALSE)
	{
		...
		{
			// Cast to float to support bigint data type
			if ($return = $this->tenantDb->delete($this->tables['users_groups'], [$this->join['users'] => (float)$user_id]))
			{
	...
	public function groups()
	{
		...
		if (isset($this->_ion_orwhere) && !empty($this->_ion_orwhere))
		{
			foreach ($this->_ion_orwhere as $orwhere)
			{
				$this->tenantDb->or_where($orwhere);
			}
			$this->_ion_orwhere = [];
		}
		...
		$this->response = $this->tenantDb->get($this->tables['groups']);
	 ...
	 * @author Phil Sturgeon
	 */
	public function update($id, array $data,$menuUpdate=false)
	{
		$this->trigger_events('pre_update_user');

		// CRITICAL FIX: Start transaction and lock row to prevent concurrent password updates
		// If same user is logged in on multiple devices and both try to update password,
		// row locking prevents race conditions that could cause password swapping
		$this->tenantDb->trans_begin();
		
		// Lock the row FOR UPDATE to prevent concurrent modifications from multiple devices
		// This ensures only one device can update password at a time
		...
		if (array_key_exists($this->identity_column, $data) || array_key_exists('password', $data) || array_key_exists('email', $data))
		{
			if (array_key_exists('password', $data))
			{
				if( ! empty($data['password']))
				{
					...
					log_message('info', "PASSWORD UPDATE VIA USER MANAGEMENT: User ID={$id}, Identity={$user->{$this->identity_column}}, Username=" . ($user->username ?? 'UNKNOWN') . ", Initiated by IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
					
					// Process password (hash_password now just trims and returns plain text)
					$data['password'] = $this->hash_password($data['password'], $user->{$this->identity_column});
					
					if ($data['password'] === FALSE)
					{
						log_message('error', "PASSWORD UPDATE PROCESSING FAILED: User ID={$id}, Identity={$user->{$this->identity_column}}, Username=" . ($user->username ?? 'UNKNOWN') . ", IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
						$this->tenantDb->trans_rollback();
						$this->trigger_events(['post_update_user', 'post_update_user_unsuccessful']);
						$this->set_error('update_unsuccessful');
						return FALSE;
					...
					log_message('info', "PASSWORD UPDATE PREPARED: User ID={$id}, Identity={$user->{$this->identity_column}}, Username=" . ($user->username ?? 'UNKNOWN') . ", Plain text password will be saved to DB, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
				}
				else
				{
					// unset password so it doesn't effect database entry if no password passed
					$this->load->helper('custom'); // Load custom helper for Australia timezone functions
					log_message('info', "PASSWORD UPDATE SKIPPED: User ID={$id}, Empty password provided, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
					unset($data['password']);
				}
			}
		}
		...
		// ✅ CRITICAL FIX: Reset query builder and use explicit WHERE clause to prevent password corruption
		// If extra_where trigger adds WHERE conditions, they could persist and affect UPDATE query
		// Using explicit where() ensures we ONLY update the specific user ID, not other users
		// This prevents password corruption during concurrent updates
		$this->tenantDb->reset_query();
		$this->tenantDb->where('id', $id);
		$this->tenantDb->update($this->tables['users'], $data);
		
		// ✅ CRITICAL SAFETY CHECK: Verify we only updated 1 row (the correct user)
		if ($this->tenantDb->affected_rows() != 1) {
			log_message('error', "🚨 CRITICAL: update() affected {$this->tenantDb->affected_rows()} rows instead of 1! User ID={$id}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		}
		if($menuUpdate==false){
	 ...
	 */
	 ...
	 // Controller method for handling user location updates
    ...
    // $existingLocationIds = $this->tenantDb->select('location_id')
    //     ->where('user_id', $userId)
    //     ->get('Global_users_to_location')
    //     ->result_array();

    // Compare the selected locations with existing ones and update as needed
    
// echo "<pre>"; print_r($existingLocationIds); exit;
//     foreach ($existingLocationIds as $existing) {
//         if (!in_array(['location_id' => $existing['location_id']], $selectedLocationIds)) {
//             // Location was associated but not selected, delete association
            $this->tenantDb->where('user_id', $userId)->delete('Global_users_to_location');
                
                
//         }
//     }
foreach ($selectedLocationIds as $locationId) {
      $this->tenantDb->insert('Global_users_to_location', ['user_id' => $userId, 'location_id' => $locationId]);
    }
	...
	public function delete_user($id)
	{
		...
		$this->tenantDb->delete($this->tables['users'], ['id' => $id]);
	...
	public function update_last_login($id)
	{
		...
		// This function runs on EVERY login. If 2 users login simultaneously (or same
		// user in 2 tabs), both try to UPDATE the same row at the same time!
		// Without locking, database writes collide and can corrupt OTHER fields like password!
		// ═══════════════════════════════════════════════════════════════════════
		...
		// CRITICAL FIX: Reset query builder IMMEDIATELY after extra_where trigger
		// extra_where may add WHERE conditions that persist and corrupt UPDATE queries
		// This MUST be done BEFORE any queries to prevent WHERE clause persistence bugs
		$this->tenantDb->reset_query();
		...
		$this->tenantDb->where('id', $id);
		$user_check = $this->tenantDb->get($this->tables['users'])->row();
		
		$this->load->helper('custom'); // Load custom helper for Australia timezone functions
		
		if (!$user_check) {
			$this->tenantDb->trans_rollback();
			log_message('error', "UPDATE LAST LOGIN FAILED: User not found. User ID={$id}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
			return FALSE;
		}
		
		$username = $user_check->username ?? 'UNKNOWN';
		
		// ✅ CRITICAL FIX: Use explicit WHERE clause to prevent password corruption
		// The third parameter of update() can be unreliable if query builder has persistent WHERE clauses
		// Using explicit where() ensures we ONLY update the correct user's last_login field
		// This prevents accidentally updating password or other fields
		$this->tenantDb->reset_query();
		$this->tenantDb->where('id', $id);
		$this->tenantDb->update($this->tables['users'], ['last_login' => time()]);
		
		// ✅ CRITICAL SAFETY CHECK: Verify we only updated 1 row (the correct user)
		if ($this->tenantDb->affected_rows() != 1) {
			log_message('error', "🚨 CRITICAL: update_last_login affected {$this->tenantDb->affected_rows()} rows instead of 1! User ID={$id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		}
		...
		if ($this->tenantDb->trans_status() === FALSE) {
			log_message('error', "UPDATE LAST LOGIN TRANSACTION FAILED: User ID={$id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
			return FALSE;
		}
		
		if ($affected == 1) {
			log_message('info', "UPDATE LAST LOGIN SUCCESS: User ID={$id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		}
	...
	public function remember_user($identity)
	{
		...
		{
			$this->tenantDb->update($this->tables['users'],
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->tenantDb->select($this->identity_column . ', id, email, remember_code, last_login')
		...
		if ($query->num_rows() === 1)
		{
			...
			// DISABLED: Session regeneration causes conflicts with concurrent logins
			// See note in login() function above for details
			// $this->session->sess_regenerate(FALSE);
	...
	public function create_group($group_name = FALSE,$group_description = '', $additional_data = [])
	{
		...
		$location_id = $this->session->userdata('location_id') ? $this->session->userdata('location_id') : ($this->session->userdata('default_location_id') ? $this->session->userdata('default_location_id') : 0);
		
		$existing_group = $this->tenantDb->get_where($this->tables['groups'], ['name' => $group_name,'location_id' =>$location_id])->num_rows();
		if($existing_group !== 0)
		{
		...
		$data = ['name'=>$group_name,'description'=>$group_description,'location_id' =>$location_id];
		...
		$this->tenantDb->insert($this->tables['groups'], $data);
		$group_id = $this->tenantDb->insert_id($this->tables['groups'] . '_id_seq');
	...
	public function update_group($group_id = FALSE, $group_name = FALSE, $additional_data = [])
	{
		...
		if (!empty($group_name))
		{
			...
			// bail if the group name already exists
			$existing_group = $this->tenantDb->get_where($this->tables['groups'], ['name' => $group_name])->row();
		...
		$group = $this->tenantDb->get_where($this->tables['groups'], ['id' => $group_id])->row();
		...
		$this->tenantDb->update($this->tables['groups'], $data, ['id' => $group_id]);
	...
	public function delete_group($group_id = FALSE)
	{
		...
		$this->tenantDb->delete($this->tables['users_groups'], [$this->join['groups'] => $group_id]);
		// remove the group itself
		$this->tenantDb->delete($this->tables['groups'], ['id' => $group_id]);
	...
	protected function _set_password_db($identity, $password)
	{
		...
		// Log password update initiation
		log_message('info', "PASSWORD UPDATE INITIATED: Identity={$identity}, Source IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		
		// Get plain text password (hash_password now just trims and returns the password)
		$plain_password = $this->hash_password($password, $identity);

		if ($plain_password === FALSE)
		{
			log_message('error', "PASSWORD UPDATE FAILED: Failed to process password for identity={$identity}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
			return FALSE;
		}
		...
		// START TRANSACTION with row lock to prevent concurrent updates
		$this->tenantDb->trans_start();
		
		// Lock the row for update (prevents concurrent modifications)
		$this->tenantDb->where($this->identity_column, $identity);
		$user_check = $this->tenantDb->get($this->tables['users'])->row();
		
		if (!$user_check) {
			$this->tenantDb->trans_rollback();
			log_message('error', "PASSWORD UPDATE FAILED: User not found for identity={$identity}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
			return FALSE;
		}
		...
		// ✅ CRITICAL FIX: Use explicit WHERE clause to prevent password corruption
		// Using identity_column (email) can cause issues if there are duplicate emails or identity mixups
		// Always use the unique ID that was just retrieved to ensure we update the correct user
		// Reset query builder to prevent WHERE clause persistence from extra_where trigger
		// Use explicit where() instead of third parameter for maximum safety
		$this->tenantDb->reset_query();
		$this->tenantDb->where('id', $user_id);
		$this->tenantDb->update($this->tables['users'], $data);
		
		// ✅ CRITICAL SAFETY CHECK: Verify we only updated 1 row (the correct user)
		if ($this->tenantDb->affected_rows() != 1) {
			log_message('error', "🚨 CRITICAL: _set_password_db() affected {$this->tenantDb->affected_rows()} rows instead of 1! User ID={$user_id}, Identity={$identity}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		}
		...
		if ($this->tenantDb->trans_status() === FALSE) {
			log_message('error', "PASSWORD UPDATE TRANSACTION FAILED: Identity={$identity}, User ID={$user_id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
			return FALSE;
		}
		
		if ($affected == 1) {
			log_message('info', "PASSWORD UPDATE SUCCESS: Identity={$identity}, User ID={$user_id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		} else {
			log_message('warning', "PASSWORD UPDATE: No rows affected for identity={$identity}, User ID={$user_id}, Username={$username}, IP=" . $this->input->ip_address() . ", Timestamp=" . australia_datetime());
		}
	...
	{
		$filtered_data = [];
		$columns = $this->tenantDb->list_fields($table);
	...
	protected function _password_verify_sha1_legacy($identity, $password, $hashed_password_db)
	{
		...
		if ($this->config->item('store_salt', 'ion_auth'))
		{
			...
			$query = $this->tenantDb->select('salt')
							  ->where($this->identity_column, $identity)
							  ->limit(1)
							  ->get($this->tables['users']);
	 ...
	 public function get_users_with_location($locationId) {
        return $this->tenantDb
            ->select('*')
            ->from('Global_roles')
            ->where('(location_id = 0 AND name = "staff") OR location_id = ' . $locationId.' AND showSeprateChecklist = 1 ')
            ->get()
            ->result_array();
    }
}
